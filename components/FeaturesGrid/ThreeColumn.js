/**
 * Features Grid - Three Column Variant
 * Grid layout with 3 columns of features
 */
...
export default function FeaturesThreeColumn({ config = {}, content = {}, globalConfig = {} }) {
  ...
  const containerStyle = {
    ...
    backgroundColor: colorScheme.background || '#f9fafb',
    textAlign: 'center',
    ...customStyle,
  ...
  return (
    <section style={containerStyle} className="features-grid-3col component-enter">
      {heading && (
        <header>
        ...
        {features.map((feature, index) => (
          <div key={feature.title || feature.heading || index} style={activeCardStyle} className="feature-card">
            {showIcons && feature.icon && (
              <div style={iconStyle}><FeatureIcon name={feature.icon} size="2.5rem" /></div>
            )}
