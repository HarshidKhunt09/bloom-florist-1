...
export default function ArticlePage({ article, pageComponents, globalConfig, allArticles, menuItems }) {
  ...
  return (
    <div className={useNewSystem ? "page-wrapper" : "container"} style={useNewSystem ? undefined : containerStyles}>
      ...
      {useNewSystem && pageComponents ? (
        // NEW: Use component system with page-specific component config
        ...
        // Component system active but no page components — article detail page
        <ArticleDetail
          article={article}
          globalConfig={globalConfig}
          allArticles={allArticles}
            ...
            {attributes.imageUrl && (
              <div>
                ...
                {attributes.photographerName && (
                  <p>
                    Photo by{" "}
                    <a href={attributes.photographerProfileUrl}>
                      {attributes.photographerName}
                    </a>{" "}
                    on Unsplash
                  </p>
...
export async function getStaticPaths() {
  try {
    ...
    const articlesForDomain = allArticles.filter(
      (article) =>
        article.attributes.Domain === config.HARDCODED_DOMAIN &&
        article.attributes.urlSlug !== "/"
    );

    const paths = articlesForDomain.map((article) => ({
      params: { slug: article.attributes.urlSlug.replace(/^\/+/, "") },
    }));

    return { paths, fallback: false };
  } catch (error) {
    console.error("Error in getStaticPaths, using fallback paths:", error?.message || error);
    const paths = fallbackOtherArticles.map((article) => ({
      params: { slug: article.attributes.urlSlug.replace(/^\/+/, "") },
    }));
    return { paths, fallback: false };
  }
...
export async function getStaticProps({ params }) {
  try {
    ...
    const article = allArticles.find(
      (article) =>
        article.attributes.Domain === config.HARDCODED_DOMAIN &&
        (article.attributes.urlSlug === slug ||
          article.attributes.urlSlug === params.slug)
    );
    ...
    try {
      if (config.PAGE_COMPONENTS_API && config.PAGE_COMPONENTS_API !== "0") {
        const componentsRes = await axios.get(
          `${config.PAGE_COMPONENTS_API}&filters[page][$eq]=${slug.replace(
            /^\//,
            ""
          )}`,
    ...
    console.error("Error in getStaticProps [slug], using fallback:", error?.message || error);
    // Find a matching fallback article by slug, or use first available
    const slug = "/" + params.slug;
    const fallbackArticle = fallbackOtherArticles.find(
      (a) => a.attributes.urlSlug === slug
    ) || fallbackOtherArticles[0];
